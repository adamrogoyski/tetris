...
declare -i current_piece=$(((RANDOM % NUM_TETROMINOS) + 1))
declare -i next_piece=$(((RANDOM % NUM_TETROMINOS) + 1))
...
starting_positions[1]='-1,0 -1,1 0,1 1,1' # Leftward L piece.
starting_positions[2]='-1,1 0,1 0,0 1,0'  # Rightward Z piece.
starting_positions[3]='-2,0 -1,0 0,0 1,0' # Long straight piece.
starting_positions[4]='-1,1 0,1 0,0 1,1'  # Bump in middle piece.
starting_positions[5]='-1,1 0,1 1,1 1,0'  # L piece.
starting_positions[6]='-1,0 0,0 0,1 1,1'  # Z piece.
starting_positions[7]='-1,0 -1,1 0,0 0,1' # Square piece.
...
function rotate() {
  declare -a new_coords
  declare -i i=0
  for dxdy in ${=rotations[$((current_piece - 1)),${current_orientation}]}; do
    declare -i dx=$((${dxdy%%,*}))
    declare -i dy=$((${dxdy##*,}))
    declare -i x=$((${current_coords[$((i + 1))]%%,*}))
    declare -i y=$((${current_coords[$((i + 1))]##*,}))
    declare -i nx=$((x + dx))
    declare -i ny=$((y + dy))
    declare xy="${nx},${ny}"
    new_coords+=("${xy}")
...
function poll_keyboard() {
  chars=${1:-1}
  IFS= read -k${chars} -t0 -r key_buffer
}

declare -ir logobig_width=66
declare -ir logobig_height=8
declare -a logobig
logobig[1]='*TTTTTTTTTT   *EEEEEEE   *TTTTTTTTTT    *RRRRRR    *II     *SSSSS'
logobig[2]='*TTTTTTTTTT   *EEEEEEE   *TTTTTTTTTT   *RR   RR    *II   *SSSS'
logobig[3]='    *TT       *E             *TT      *RR    RR    *II   *SS'
logobig[4]='    *TT       *EEEEEEE       *TT       *RR   RR    *II     *SSSS'
logobig[5]='    *TT       *EEEEEEE       *TT         *RRRRR    *II        *SSS'
logobig[6]='    *TT       *E             *TT       *RR   RR    *II         *SS'
logobig[7]='    *TT       *EEEEEEE       *TT      *RR    RR    *II      *SSSS'
logobig[8]='    *TT       *EEEEEEE       *TT     *RR     RR    *II    *SSSS '

declare -ir logosml_width=42
declare -ir logosml_height=5
declare -a logosml
logosml[1]='*TTTTTT *EEEEE *TTTTTT   *RRRR  *II   *SSS'
logosml[2]='  *TT   *E       *TT   *RR  RR  *II  *S'
logosml[3]='  *TT   *EEEEE   *TT      *RRR  *II   *SSS'
logosml[4]='  *TT   *E       *TT    *R  RR  *II     *S'
logosml[5]='  *TT   *EEEEE   *TT  *RR   RR  *II  *SSS'
...
\r
  ...
  if [[ ${status_width} -ge ${logobig_width} ]]; then
    declare -i i
    for ((i=1; i <= logobig_height; i++)); do
      echo -ne $'\e'"[$((i));${status_left}H${FRED}${logobig[${i}]}"
    done
    status_height="$((logobig_height + 2))"
  elif [[ ${status_width} -ge ${logosml_width} ]]; then
    declare -i i
    for ((i=1; i <= logosml_height; i++)); do
      echo -ne $'\e'"[$((i));${status_left}H${FRED}${logosml[${i}]}"
    done
...
start_ms="${start_ms%%.*}$(echo ${start_ms##*.} | cut -c1-3)"
function time_ms() {
  declare now_ms
  now_ms="${EPOCHREALTIME}"
  now_ms="${now_ms%%.*}$(echo ${now_ms##*.} | cut -c1-3)"
  ...
  if [[ "${state}" == "play" ]]; then
    ...
    if ((game_ticks >= drop_ticks + $((drop_speed < 1 ? 1 : drop_speed)))); then
      ...
      if ! boundary_condition 0 1 && ! colission_detected 0 1; then
        ...
        next_piece=$(((RANDOM % NUM_TETROMINOS) + 1))
