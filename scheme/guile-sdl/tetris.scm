...
(define (draw-screen)
  ...
  (draw-board board 0)
  (draw-wall 0)
  (let ((status-x (+ (* board.block.width square-size) 50))
    ...
    (render-copy renderer logo #:dstrect (make-rect status-x 20 (- width-px status-x) (* 2 square-size)))
    (render-copy renderer lines-texture #:dstrect (make-rect status-x (+ (* 2 square-size) 100) 120 30))
    (render-copy renderer level-texture #:dstrect (make-rect status-x (+ (* 2 square-size) 180) 120 30))
    ...
    (let ((coords (copy-tree (list-ref starting-positions next-piece-type)))
          ...
          (place-x (* (+ board.block.width 3) square-size)))
      (for-each (lambda (coord)
                  (let ((x (car  coord))
                        (y (cadr coord)))
                    (render-copy renderer block #:dstrect (make-rect (+ (* (- x left-adjust-x) square-size) place-x)
                                                                     (* (+ y (max 4 (- (/ board.block.height 2) 1))) square-size)
