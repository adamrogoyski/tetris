...
import sdl = require('@kmamal/sdl');
import canvas = require('canvas');
import sound = require('node-aplay');
...
class GameContext {
  ...
  block_black: canvas.Image;
  block_blue: canvas.Image;
  block_cyan: canvas.Image;
  block_green: canvas.Image;
  block_orange: canvas.Image;
  block_purple: canvas.Image;
  block_red: canvas.Image;
  block_yellow: canvas.Image;
  blocks: canvas.Image[];
...
const ctx: GameContext = new GameContext();
...
function Init() {
  ...
  ctx.logo = new canvas.Image();
  ctx.logo.src = 'graphics/logo.png';
  ctx.wall = new canvas.Image();
  ...
  if (process.argv.length >= 2) {
    let level = parseInt(process.argv[process.argv.length-1]) * 3;
    if (!isNaN(level)) {
...
function DrawStatus(sctx) {
  ...
  for (let h = 0; h < HEIGHT_PX; h += Math.min(HEIGHT_PX - h, 640)) {
...
function ClearBoard() {
  var rows_deleted = 0;
  for (let row = HEIGHT - 1; row >= rows_deleted;) {
    var has_hole = false;
    for (let x = 0; x < WIDTH && !has_hole; ++x) {
      has_hole = !ctx.board[row][x];
    }
    if (!has_hole) {
      var deleted_row = ctx.board[row];
      for (let y = row; y > rows_deleted; --y) {
