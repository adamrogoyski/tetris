...
# A tetris game.
#
# The only non-bash dependency is stty for the game.
# For music, cat and tee are used, along with the pulseaudio wrapper.
...
# Start music.
(while :; do cat sound/audio.raw 2>/dev/null | LD_PRELOAD=/usr/lib/x86_64-linux-gnu/pulseaudio/libpulsedsp.so tee /dev/audio >& /dev/null; done) 2>/dev/null &
